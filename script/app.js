...
const currentPath = document.body.dataset.route || '/'
const depthPrefix = document.body.dataset.depth === 'inner' ? '../' : ''
...
function getPageHref(path) {
  ...
  return document.body.dataset.depth === 'inner' ? file : `html/${file}`
}
...
function renderNavLink(item) {
  ...
  return `
    <a class="nav-link${activeClass}" href="${getPageHref(item.to)}">
...
function renderSubNavLink(item) {
  ...
  return `
    <a class="nav-link${activeClass}" href="${getPageHref(item.path)}">
...
function renderActionButton(button) {
  ...
  return `
    <a class="arcade-button" href="${getPageHref(button.to)}">
      ${escapeHtml(button.label)}
    </a>
  `
}
  ...
  return `
    <header class="topbar">
      <section class="topbar-main panel">
        <a class="brand" href="${getPageHref('/')}">
      ...
      <section class="cv-sheet panel">
        <header class="cv-header">
          ...
          <div class="button-row cv-header-actions">
            ...
            <a class="arcade-button ghost" href="${getPageHref('/contact')}">Ga naar contact</a>
          </div>
        </header>
    ...
    return `
      <div class="page-shell">
        <section class="panel rich-content">
          ...
          <a class="arcade-button" href="${getPageHref('/')}">Terug naar home</a>
        </section>
      </div>
    `
  }
